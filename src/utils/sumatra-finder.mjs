 ...
 * @returns {Object} Object containing path and filename of SumatraPDF
...
export function findSumatraPDF(options = {}) {
  ...
  if (fs.existsSync(pdfPrinterDistDir)) {
    try {
      ...
      if (sumatraFile) {
        return {
          path: path.join(pdfPrinterDistDir, sumatraFile),
          filename: sumatraFile
        };
      }
    } catch (error) {
  ...
  if (fs.existsSync(resourcesDir)) {
    try {
      ...
      if (sumatraFile) {
        return {
          path: path.join(resourcesDir, sumatraFile),
          filename: sumatraFile
        };
      }
    } catch (error) {
